...
int xDBLMUL_vec(ec_point_t *res,
                        const digit_t (*scalar_vec)[NWORDS_ORDER],
                        const int kbits,
                        const ec_basis_t *PQ,
                        const ec_curve_t *curve)
{
    ...
    bitk0 = (scalar_vec[0][0] & 1);
    bitl0 = (scalar_vec[1][0] & 1);
    ...
    one[0] = 1;
    mp_sub(k_t, scalar_vec[0], one, NWORDS_ORDER);
    mp_sub(l_t, scalar_vec[1], one, NWORDS_ORDER);

    select_ct(k_t, k_t, scalar_vec[0], maskk, NWORDS_ORDER);
    select_ct(l_t, l_t, scalar_vec[1], maskl, NWORDS_ORDER);
    ...
    ec_point_init(&R[0]);
    maskk = 0 - sigma[0];
    select_point(&R[1], &PQ->P, &PQ->Q, maskk);
    select_point(&R[2], &PQ->Q, &PQ->P, maskk);
    ...
    fp2_copy(&DIFF2b.x, &PQ->PmQ.x);
    fp2_copy(&DIFF2b.z, &PQ->PmQ.z);
    ...
    // Output R[evens]
    select_point(res, &R[0], &R[1], mevens);
    maskk = 0 - (bitk0 & bitl0);
    select_point(res, res, &R[2], maskk);
    return 1;
}

void ec_basis_scalar_mul_vec(ec_basis_t *out, int e, const int *n, const ec_basis_t *input, const ec_curve_t *curve)
{
    digit_t tmp1[2][NWORDS_ORDER] = {0};
    tmp1[0][0] = n[0];
    xDBLMUL_vec(&out->P, tmp1, e, input, curve);

    digit_t tmp2[2][NWORDS_ORDER] = {0};
    tmp2[1][0] = n[0];
    xDBLMUL_vec(&out->Q, tmp2, e, input, curve);

    tmp1[1][0] = 18446744073709551615ULL - n[0] + 1;
    tmp1[1][1] = tmp1[1][2] = 18446744073709551615ULL;
    tmp1[1][3] = 72057594037927935;
    //printf("last:\n");
    xDBLMUL_vec(&out->PmQ, tmp1, e, input, curve);
}
